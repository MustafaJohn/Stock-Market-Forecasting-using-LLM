...
mse = mean_squared_error(results['ExcessReturn'], results['PredictedReturns'])
r2 = r2_score(results['ExcessReturn'], results['PredictedReturns'])
...
alpha = 0.001
estimation_window = 252  # set here, avoid overriding inside function
...
results_df.to_csv('model_results.csv (252-window).csv', index=False)
# Save predictions to CSV
wide_preds_df.sort_values(['DlyCalDt', 'PERMNO'], inplace=True)
wide_preds_df.to_csv('model_predictions.csv (252-window).csv', index=False)
